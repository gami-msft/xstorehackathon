...
namespace Hackathon
{
    class SetAclRecursiveMillionBlob
    {
        public static void Execute(bool redirectOutputToFile)
        {
            Console.WriteLine("Code demonstrating setting acls on a million blobs [in random order].");

            Helper helper = new Helper();

            // Set output path for debugging purposes
            helper.SetConsoleOutPutPath(redirectOutputToFile, ".\\SetAclRecursiveMillionRandom.txt");
            ...
            SetAclRecursivetMillionBlobsRandomOrder(fileSystemClient, Helper.DirectoryForListing);

            Console.WriteLine("Code demonstrating setting acls on a million blobs [completed successfully].");
        }

        /// <summary>
        /// Function to set acl recursively on one million blobs in random order
        /// Parrallelization can be controlled via 'numTasks'
        /// </summary>
        public static void SetAclRecursivetMillionBlobsRandomOrder(DataLakeFileSystemClient fileSystemClient, string directoryName)
        {
